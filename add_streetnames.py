...
def add_streetnames_with_projection(json_file, output_json, max_distance_meters=50):
    """
    Fügt Straßennamen hinzu mit korrekter Koordinatentransformation
    """
    print("🚗 Füge Straßennamen mit Koordinatentransformation hinzu...")
    ...
    # 3. Straßennetz laden
    print("🗺️ Lade Straßennetz...")
    streets = gpd.read_file("SWISSTLM3D_2025.gpkg", layer="tlm_strassen_strasse")
    
    # Prüfe Koordinatensystem der Straßen
    print(f"📊 Straßen-CRS: {streets.crs}")
    
    # Nur Straßen mit Namen
    streets_with_names = streets[
        (streets['strassenname'].notna()) | 
        (streets['name'].notna())
    ].copy()
    
    print(f"✅ {len(streets_with_names)} Straßen mit Namen geladen")
    
    # Spatial Index für Performance
    print("⚡ Erstelle Spatial Index...")
    sindex = streets_with_names.sindex
    
    # 4. Straßennamen zu Unfällen hinzufügen
    print("🔍 Finde Straßennamen...")
    ...
    print(f"📊 {matches_found}/{len(accidents)} Unfälle mit Straßennamen")
...
def check_streets_coordinates():
    """
    Prüft die Koordinaten der Straßen
    """
    print("\n🔍 Prüfe Straßen-Koordinaten...")
    ...
    print(f"🏙️  Straßen in Zürich Bereich: {len(zurich_streets)}")
